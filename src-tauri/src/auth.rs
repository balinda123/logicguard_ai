...
fn hash_password(password: &str) -> Result<String, String> {
    if password.len() < 8 { return Err("密码至少需要 8 个字符".to_string()); }
    let salt = SaltString::generate(&mut OsRng);
    Argon2::default().hash_password(password.as_bytes(), &salt)
        .map(|hash| hash.to_string()).map_err(|e| e.to_string())
}

fn require_admin() -> Result<SessionUser, String> {
    let user = current_user()?;
    if user.role != "admin" { return Err("仅管理员可以执行此操作".to_string()); }
    Ok(user)
}

pub fn current_user() -> Result<SessionUser, String> {
    session().lock().map_err(|_| "会话锁异常".to_string())?
        .clone().ok_or_else(|| "AUTH_REQUIRED: 请先登录".to_string())
}

pub fn current_user_id() -> Result<String, String> { Ok(current_user()?.id) }

pub fn current_api_key() -> Result<String, String> {
    let user = current_user()?;
    keyring::Entry::new(CREDENTIAL_SERVICE, &user.id)
        .map_err(|e| format!("凭据库初始化失败: {}", e))?
        .get_password().map_err(|_| "KEY_MISSING: 请在系统设置中配置 API Key".to_string())
}
...
pub fn auth_status(app: tauri::AppHandle) -> Result<AuthStatus, String> {
    ...
    let user = session().lock().map_err(|_| "会话锁异常".to_string())?.clone();
    Ok(AuthStatus { initialized: count > 0, user })
}

#[tauri::command]
pub fn initialize_admin(app: tauri::AppHandle, username: String, password: String) -> Result<SessionUser, String> {
    let conn = open_db(&app)?;
    let count: i64 = conn.query_row("SELECT COUNT(*) FROM users", [], |r| r.get(0)).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn login(app: tauri::AppHandle, username: String, password: String) -> Result<SessionUser, String> {
    let conn = open_db(&app)?;
    let row: Option<(String,String,String,String,i64)> = conn.query_row(
        ...
        [username.trim()], |r| Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?,r.get(4)?)),
...
pub fn disable_user(app: tauri::AppHandle, user_id: String) -> Result<(), String> {
    ...
    if admin.id == user_id { return Err("不能禁用当前管理员".to_string()); }
    open_db(&app)?.execute("UPDATE users SET disabled=1 WHERE id=?1", [user_id]).map_err(|e| e.to_string())?;
    Ok(())
}

#[tauri::command]
pub fn reset_user_password(app: tauri::AppHandle, user_id: String, password: String) -> Result<(), String> {
    require_admin()?;
    let hash = hash_password(&password)?;
    open_db(&app)?.execute("UPDATE users SET password_hash=?1 WHERE id=?2", params![hash,user_id]).map_err(|e| e.to_string())?;
    Ok(())
}

#[tauri::command]
pub fn save_api_key(api_key: String) -> Result<(), String> {
    if api_key.trim().len() < 10 { return Err("API Key 格式不正确".to_string()); }
    let user = current_user()?;
    keyring::Entry::new(CREDENTIAL_SERVICE, &user.id).map_err(|e| e.to_string())?
        .set_password(api_key.trim()).map_err(|e| format!("保存系统凭据失败: {}", e))
}

#[tauri::command]
pub fn credential_status() -> Result<bool, String> { Ok(current_api_key().is_ok()) }
