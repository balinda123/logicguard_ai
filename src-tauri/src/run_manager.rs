...
pub fn recover_interrupted(conn: &Connection) -> Result<usize, String> {
    conn.execute(&format!("UPDATE execution_runs SET status='interrupted',error_category='interrupted',error_message='Application restarted without a recoverable worker',worker_pid=NULL,lease_owner=NULL,lease_expires_at=NULL,finished_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE status IN ({})", ACTIVE_STATUSES.iter().map(|_| "?").collect::<Vec<_>>().join(",")), rusqlite::params_from_iter(ACTIVE_STATUSES.iter().copied())).map_err(|e|e.to_string())
}
...
#[derive(Clone)] pub struct RunManager { app:AppHandle, db_path:PathBuf, leases:Arc<Mutex<LeaseCoordinator>>, controls:Arc<Mutex<HashMap<String,Arc<ControlEntry>>>> }

impl RunManager {
    pub fn new(app:AppHandle,db_path:PathBuf)->Result<Self,String>{ let conn=Connection::open(&db_path).map_err(|e|e.to_string())?; initialize_schema(&conn)?; recover_interrupted(&conn)?; Ok(Self{app,db_path,leases:Default::default(),controls:Default::default()}) }
    ...
    fn transition(&self,id:&str,to:RunStatus,category:Option<&str>,message:Option<&str>)->Result<ExecutionRun,String>{
        ...
        let run=load_run(&conn,id)?.ok_or("NOT_FOUND")?; self.emit_run(&run); self.record_event(id,to.as_str(),json!({"status":to}))?; Ok(run)
    }
    ...
    async fn execute_queued(&self,id:String){
        ...
        if let Ok(mut leases)=self.leases.lock(){leases.release(&id)}; if let Ok(conn)=self.db(){let _=conn.execute("UPDATE execution_runs SET worker_pid=NULL,lease_owner=NULL,lease_expires_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?1",[&id]);}
        self.spawn_next();
    }
    ...
    async fn execute_with_lease(&self,id:&str)->Result<(),String>{
        ...
        for (index,command) in run.execution_plan.commands.iter().enumerate().skip(run.current_step as usize){
            ...
            if cancelled { worker.terminate().await; return Ok(()); }
            let mut attempt=1; loop { match worker.send(index,command,&control.cancelled,|data|{let _=self.record_event(id,"progress",json!({"step":index,"data":data}));}).await {
                ...
                Err(failure)=>match policy.decision(attempt,failure.category){
                    RetryDecision::RetryAfter(delay)=>{self.record_event(id,"retry",json!({"step":index,"attempt":attempt,"category":failure.category.as_str()}))?;worker.terminate().await;tokio::time::sleep(Duration::from_millis(delay)).await;worker=WorkerProcess::spawn(&self.app).await.map_err(|e|sanitize(&e))?;self.persist_worker(id,&worker)?;attempt+=1;},
                    ...
                    RetryDecision::Cancelled=>{worker.terminate().await;self.transition(id,RunStatus::Cancelled,Some("cancelled"),Some(&failure.message))?;return Ok(())},
            ...
            match decision{
                ControlDecision::Continue=>{},ControlDecision::PauseAt(_)=>{worker.terminate().await;self.transition(id,RunStatus::Paused,None,None)?;return Ok(())},ControlDecision::Cancel=>{worker.terminate().await;return Ok(())}
            }
        }
        worker.terminate().await;self.transition(id,RunStatus::Passed,None,None)?;Ok(())
    }
    ...
    pub fn terminate(&self,id:&str)->Result<ExecutionRun,String>{let run=self.owned(id)?;if run.status.is_terminal(){return Ok(run)};if let Ok(control)=self.control(id){if let Ok(mut value)=control.state.lock(){value.cancel();}control.cancelled.notify_one();}self.transition(id,RunStatus::Cancelled,Some("cancelled"),Some("Terminated by user"))}
}
