import { Eye, LockKeyhole, Pause, Play, Square, TerminalSquare } from 'lucide-react'
...
export function ActiveRunBar({ onOpenExecution }: { onOpenExecution: () => void }) {
  ...
  return <section aria-label="活动执行控制" className="shrink-0 border border-warning/40 bg-warning/10 px-4 py-3 shadow-md">
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
      <div className="flex min-w-52 items-center gap-2 text-warning">
        <span className="relative flex h-3 w-3"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-warning opacity-50" /><span className="relative inline-flex h-3 w-3 rounded-full bg-warning" /></span>
        <strong className="text-sm">自动化执行中</strong>
      ...
      <div className="flex items-center gap-2 text-[11px] text-warning"><LockKeyhole className="h-3.5 w-3.5" />浏览器受控，暂停后才可手动操作</div>
      <div className="flex items-center gap-1.5">
        ...
        <button type="button" title="打开执行中心" aria-label="打开执行中心" onClick={() => { setSelectedRunId(run.id); onOpenExecution() }} className="flex h-8 w-8 items-center justify-center rounded-md border border-border bg-surface-1 text-text-secondary hover:text-brand-400"><TerminalSquare className="h-3.5 w-3.5" /></button>
