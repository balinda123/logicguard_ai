...
function roleLabel(role?: BusinessRole): string {
  return role ? ROLE_LABEL[role] : '未标记'
}
...
export function IssueTracker({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const filteredDrafts = useMemo(() => {
    ...
    return drafts.filter(draft => {
      ...
      const age = filterNow - new Date(draft.createdAt).getTime()
  ...
  return (
    <div className="flex h-full flex-1 flex-col overflow-hidden p-6">
      ...
      <section className="flex shrink-0 flex-wrap items-end gap-3 border-b border-border py-3">
        ...
        <label className="grid gap-1 text-[11px] text-text-muted">角色筛选<select aria-label="角色筛选" value={roleFilter} onChange={event => setRoleFilter(event.target.value as 'all' | BusinessRole)} className="h-8 min-w-28 rounded-md border border-border bg-surface-2 px-2 text-xs text-text-primary"><option value="all">全部角色</option>{Object.entries(ROLE_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
