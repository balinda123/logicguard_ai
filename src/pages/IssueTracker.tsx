...
import { listExecutionIssues, updateExecutionIssue, updateExecutionIssueStatus } from '../api/runBridge'
import type { ExecutionIssue } from '../types/execution'
import type { BusinessRole, DefectDraft, DefectStatus, FailureEvidence, WorkflowScenario } from '../types/workflow'
import { downloadDefectCsv, downloadDefectWorkbook, safeEvidenceRelativePath } from '../utils/defectExport'
...
type IssueSource = 'workflow' | 'execution'
type TrackedIssue = Omit<DefectDraft, 'scenarioId' | 'evidenceId'> & {
  source: IssueSource
  scenarioId?: string
  evidenceId?: string
}
...
const STATUS_LABEL: Record<DefectStatus, string> = {
  pending_confirmation: '待确认', pending_fix: '待修复', pending_validation: '待验证', closed: '已关闭', not_a_bug: '非缺陷',
}
...
const ALLOWED_ACTIONS: Record<DefectStatus, Array<{ next: DefectStatus; label: string }>> = {
  pending_confirmation: [{ next: 'pending_fix', label: '确认提交开发' }, { next: 'not_a_bug', label: '标记非缺陷' }],
  pending_fix: [{ next: 'pending_validation', label: '转待验证' }],
  pending_validation: [{ next: 'closed', label: '关闭问题' }, { next: 'pending_fix', label: '退回待修复' }],
  closed: [], not_a_bug: [],
}

function roleLabel(role?: BusinessRole): string { return role ? (ROLE_LABEL[role] || role) : '未标记' }
...
function safePreviewUrl(appDataDir: string, relativePath?: string): string | undefined {
  ...
  return convertFileSrc(`${appDataDir.replace(/[\\/]+$/, '')}/${safePath}`)
}
function fromWorkflowIssue(item: DefectDraft): TrackedIssue { return { ...item, source: 'workflow' } }
function fromExecutionIssue(item: ExecutionIssue): TrackedIssue { return { ...item, source: 'execution' } }
...
export function IssueTracker({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const [form, setForm] = useState<TrackedIssue | null>(null)
  ...
  const filteredDrafts = useMemo(() => {
    ...
    return drafts.filter(draft => {
      ...
      return (statusFilter === 'all' || draft.status === statusFilter)
        && (roleFilter === 'all' || draft.role === roleFilter)
        && (systemFilter === 'all' || draft.systemId === systemFilter)
        && (environmentFilter === 'all' || draft.environmentId === environmentFilter)
        && filterNow - new Date(draft.createdAt).getTime() <= timeLimit
        && (!query || searchable.includes(query))
    }).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))
  }, [drafts, environmentFilter, filterNow, keyword, roleFilter, statusFilter, systemFilter, timeFilter])

  const load = async () => {
    setLoading(true)
    try {
      const [workflowDrafts, executionIssues, nextEvidence, nextScenarios, nextSystems] = await Promise.all([
        listDefectDrafts(), listExecutionIssues(), listFailureEvidence(), listWorkflowScenarios(), listSystems(),
      ])
      setDrafts([...executionIssues.map(fromExecutionIssue), ...workflowDrafts.map(fromWorkflowIssue)])
  ...
  const openDraft = (draft: TrackedIssue) => {
    setSelectedId(draft.id)
    setForm({ ...draft, reproductionSteps: [...draft.reproductionSteps] })
  }
  const updateForm = (patch: Partial<TrackedIssue>) => setForm(current => current ? { ...current, ...patch } : current)

  const saveEdits = async (): Promise<TrackedIssue | null> => {
    if (!form) return null
    const reproductionSteps = form.reproductionSteps.map(item => item.trim()).filter(Boolean)
    if (form.source === 'execution') {
      const saved = fromExecutionIssue(await updateExecutionIssue({
        id: form.id, title: form.title.trim(), reproductionSteps, expectedResult: form.expectedResult.trim(),
        actualResult: form.actualResult.trim(), impact: form.impact.trim(), role: form.role,
      }))
      setDrafts(current => current.map(item => item.id === saved.id ? saved : item))
      setForm(saved)
      return saved
    }
    const saved = fromWorkflowIssue(await saveDefectDraft({
      id: form.id, scenarioId: form.scenarioId ?? '', runId: form.runId, evidenceId: form.evidenceId,
      status: form.status, title: form.title.trim(), reproductionSteps, expectedResult: form.expectedResult.trim(),
      actualResult: form.actualResult.trim(), impact: form.impact.trim(), role: form.role,
    }))
    setDrafts(current => current.map(item => item.id === saved.id ? saved : item))
    setForm(saved)
  ...
  const applyAction = async (next: DefectStatus) => {
    ...
    try {
      ...
      if (!saved) return
      const updated = saved.source === 'execution'
        ? fromExecutionIssue(await updateExecutionIssueStatus(saved.id, next))
        : fromWorkflowIssue(await updateDefectDraftStatus(saved.id, next))
  ...
  const exportItems = filteredDrafts.map(draft => ({ draft, evidencePath: evidenceById.get(draft.evidenceId ?? '')?.screenshotPath }))
  const selectedEvidence = selected?.source === 'workflow' ? evidenceById.get(selected.evidenceId ?? '') : undefined
