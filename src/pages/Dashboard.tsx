...
export function Dashboard({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  const actions = [{ title: '测试设计', tab: 'testdesign', icon: Sparkles }, { title: '执行中心', tab: 'execution', icon: ListChecks }, { title: '测试报告', tab: 'reports', icon: ClipboardList }, { title: '问题跟踪', tab: 'issues', icon: FileWarning }]
  return <div className="flex h-full flex-1 flex-col overflow-y-auto p-6"><header className="border-b border-border pb-4"><h2 className="text-lg font-bold text-text-primary">全局测试工作台</h2><p className="mt-1 text-xs text-text-muted">这里汇总全部系统；只有测试设计按系统和环境隔离。</p></header>
